"""
Apply migration 019: Re-add cidade and estado columns to dim_empresas.

These columns were removed by migration 013 but are needed for company table display.

Usage:
    python scripts/apply_migration_019_readd_cidade_estado.py
"""
...
MIGRATION_FILE = Path(__file__).parent.parent / "backend" / "database" / "migrations" / "019_readd_cidade_estado.sql"
...
def main() -> None:
    ...
    logger.info("migration_start", file="019_readd_cidade_estado.sql", statements=len(statements))
    ...
    logger.info("migration_complete", file="019_readd_cidade_estado.sql")
