...
def main():
    ...
    try:
        ...
        result = (
            supabase.table("raw_cnae").select("id", count="exact").limit(1).execute()
        )
    ...
    try:
        ...
        result = (
            supabase.table("fato_regime_tributario")
            .select("id, cnae_principal", count="exact")
            .limit(5)
            .execute()
        )
        ...
        if result.data:
            cnaes = [
                r.get("cnae_principal") for r in result.data if r.get("cnae_principal")
            ]
            if cnaes:
