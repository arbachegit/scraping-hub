...
export function StatsBadgeCard({
  ...
  return (
    <div
      className={`
        flex flex-col rounded-lg border border-white/5 border-l-[3px] ${config.border}
        ${config.bg} backdrop-blur-sm min-w-[140px] flex-1
    ...
    >
      ...
      <div className="flex items-center justify-between px-2.5 py-1.5 border-b border-white/5">
        <div className="flex items-center gap-1.5 min-w-0">
          <Icon className={`w-3.5 h-3.5 flex-shrink-0 ${config.text}`} />
          <span className="text-[11px] font-medium text-slate-300 truncate">{label}</span>
        </div>
        <div className="flex items-center gap-1.5 flex-shrink-0">
          <span className="text-[9px] text-slate-500">{formatDate(dataReferencia)}</span>
          <div className="flex items-center gap-1">
            <div
              className={`w-1.5 h-1.5 rounded-full ${online ? 'bg-green-400 animate-pulse' : 'bg-slate-500'}`}
            />
            <CountdownRing progress={countdownProgress} color={config.line} size={14} />
          </div>
        </div>
      </div>

      {/* Body - Chart */}
      <div className="flex items-center justify-center px-2 py-1.5 min-h-[32px]">
        <MiniSparkline data={historyValues} color={config.line} width={100} height={28} />
      </div>

      {/* Footer */}
      <div className="flex items-center justify-between px-2.5 py-1.5 border-t border-white/5">
        <span className="text-sm font-bold text-white tabular-nums">
          {formatNumber(total)}
        </span>
        <div className={`flex items-center gap-0.5 text-[10px] ${growthColor}`}>
